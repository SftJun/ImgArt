...
public class UserController extends BaseController {
	...
	public String loginPost(
		...
		if (user.getPassword() != null && user.getPassword().equals(EncryptionTools.MD5(password))) {
			modelMap.addAttribute(
					AppString.SESSION_USER_ID, 
	...
	public void registerUserPost(ModelMap modelMap, HttpServletRequest httpServletRequest) {
		User user = new User();
		...
		log.info("用户注册成功");
		modelMap.addAttribute(AppString.SESSION_USER_ID, 
				userService.findUserByName(user.getUserName()).getId());
	}
}
